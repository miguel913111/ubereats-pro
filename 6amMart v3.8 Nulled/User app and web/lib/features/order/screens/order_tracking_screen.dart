...
import 'package:nexofood_user/helper/marker_helper.dart';
import 'package:nexofood_user/helper/pusher_helper.dart';
...
import 'package:nexofood_user/features/order/widgets/track_details_view_widget.dart';
import 'package:nexofood_user/features/order/widgets/tracking_stepper_widget.dart';
...
class OrderTrackingScreenState extends State<OrderTrackingScreen> with WidgetsBindingObserver {
  ...
  void setMarker(Store? store, DeliveryMan? deliveryMan, AddressModel? addressModel, bool takeAway, bool parcel, bool isRestaurant, {AddressModel? currentAddress, bool fromCurrentLocation = false}) async {
    try {
      ...
      Widget restaurantImageData = await MarkerHelper.convertAssetToBitmapDescriptor(
        width: 30, imagePath: parcel ? Images.userMarker : isRestaurant ? Images.restaurantMarker : Images.markerStore,
      );

      Widget deliveryBoyImageData = await MarkerHelper.convertAssetToBitmapDescriptor(
        width: 30, imagePath: Images.deliveryManMarker,
      );
      Widget destinationImageData = await MarkerHelper.convertAssetToBitmapDescriptor(
