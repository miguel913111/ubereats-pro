...
import 'package:nexofood_user/features/splash/domain/models/landing_model.dart';
...
import 'package:nexofood_user/helper/splash_route_helper.dart';
...
class SplashController extends GetxController implements GetxService {
  ...
  bool _prepareModuleList(List<ModuleModel>? moduleList) {
    if (moduleList != null) {
      List<ModuleModel> newModuleList = [];
      ...
      bool hasChanged = _moduleList == null || _moduleList!.length != newModuleList.length;
      if(!hasChanged && _moduleList != null) {
        for(int i = 0; i < _moduleList!.length; i++) {
          if(_moduleList![i].id != newModuleList[i].id) {
            hasChanged = true;
            break;
          }
        }
      }
      _moduleList = newModuleList;
      if(hasChanged) {
        update();
      }
      return true;
  ...
  void switchModule(int index, bool fromPhone) async {
    if(_module == null || _module!.id != _moduleList![index].id) {
      ...
      if(_module != null && _module!.moduleType.toString() != AppConstants.taxi) {
        Get.find<CartController>().getCartDataOnline();
