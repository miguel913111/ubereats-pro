...
class SearchLocationWidget extends StatelessWidget {
  ...
  const SearchLocationWidget({super.key, required this.mapController, required this.pickedAddress, required this.isEnabled, this.isPickedUp, this.hint, this.fromDialog = false, this.onLocationSelected});

  @override
  Widget build(BuildContext context) {
    return InkWell(
      onTap: () {
        Get.dialog(LocationSearchDialogWidget(mapController: mapController, isPickedUp: isPickedUp, onLocationSelected: onLocationSelected));
        if(isEnabled != null) {
