...
import 'package:nexofood_user/common/widgets/no_internet_screen.dart';
...
import 'package:nexofood_user/features/location/widgets/module_dialog_widget.dart';
...
import 'package:nexofood_user/helper/taxi_helper.dart';
...
class LocationController extends GetxController implements GetxService {
  ...
  Future<void> syncZoneData() async {
    ...
    if (address?.latitude == null || address?.longitude == null || address!.latitude == '0' || address.longitude == '0') {
      print('====syncZoneData: no valid address coordinates, skipping');
      return;
    }

    ZoneResponseModel response = await getZone(address.latitude!, address.longitude!, false, updateInAddress: true);
  ...
  Future<void> updatePosition(CameraPosition? position, bool fromAddress) async {
    ...
    if(_updateAddAddressData) {
      _loading = true;
      _changeAddress = true; // Always reset so geocode runs on user action
      update();
  ...
  void _prepareZoneData(AddressModel address, bool fromSignUp, String? route, bool canRoute, bool isDesktop) async {
    ...
    getZone(address.latitude, address.longitude, false).then((response) async {
      if (response.isSuccess) {
        ...
        //ZoneResponseModel serviceZoneModel = await Get.find<SearchLocationController>().getServiceZone(address.latitude ?? '', address.longitude ?? '');

        print('======== >>>>>>>>> rideZoneModel: ${rideZoneModel.zoneId}');
        int? parsedRideZoneId = (rideZoneModel.zoneId != null && rideZoneModel.zoneId!.isNotEmpty) ? int.tryParse(rideZoneModel.zoneId!) : null;
        int? fallbackZoneId = response.zoneIds.isNotEmpty ? response.zoneIds[0] : null;
        address.zoneId = parsedRideZoneId ?? fallbackZoneId;
       // address.serviceZoneId = serviceZoneModel.zoneId != null ? int.tryParse(serviceZoneModel.zoneId??'') : null;
        address.rideZoneId = parsedRideZoneId ?? fallbackZoneId;
        address.zoneIds = [];
        address.zoneIds!.addAll(response.zoneIds);
        address.zoneData = [];
        address.zoneData!.addAll(response.zoneData);
  ...
  Future<AddressModel> setLocationFromLatLng(LatLng latLng, String? address, GoogleMapController? mapController) async {
    _loading = true;
    update();
