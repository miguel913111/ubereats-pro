...
import 'package:nexofood_vendor/common/widgets/custom_loader_widget.dart';
...
import 'package:nexofood_vendor/features/advertisement/models/popup_menu_model.dart';
import 'package:nexofood_vendor/features/advertisement/screens/create_advertisement_screen.dart';
import 'package:nexofood_vendor/features/advertisement/widgets/confirmation_bottom_sheet.dart';
import 'package:nexofood_vendor/features/advertisement/widgets/empty_ads_view.dart';
import 'package:nexofood_vendor/helper/date_converter_helper.dart';
import 'package:nexofood_vendor/helper/route_helper.dart';
import 'package:nexofood_vendor/helper/string_extensions.dart';
    ...
    return GetBuilder<AdvertisementController>(
      builder: (adsController) {
        return Column(children: [
          ...
          Expanded(
            child: ListView.builder(
              itemCount: advertisementList!.length + 1,
                controller: scrollController,
                itemBuilder: (context, index) {
                  if (index == advertisementList.length) {
                    return const SizedBox(height: Dimensions.paddingSizeExtraOverLarge * 1.5);
                  }
                                    ...
                                    if(option.title == "edit_ads"){
                                      if (status == 'running'){
                                        Get.dialog(ConfirmationDialogWidget(
                                          ...
                                          title: 'do_you_want_to_edit'.tr,
                                          description: 'your_ad_is_running'.tr,
                                          onNoPressed: () => Get.back(),
                                          onNoButtonText: 'not_now'.tr,
                                          onYesButtonText: 'yes_edit'.tr,
                                          onYesPressed: () async {
                                            Get.back();
                                      ...
                                      } else {Get.dialog(const CustomLoaderWidget());
                                        adsController.getAdvertisementDetails(id: advertisementList[index].id!).then((AdsDetailsModel? adsDetailsModel) {
                                          Get.back();
                                          if(adsDetailsModel != null) {
                                            Get.to(()=> CreateAdvertisementScreen(adsDetailsModel: adsDetailsModel));
