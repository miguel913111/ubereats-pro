...
class OrderLogic
{
    ...
    public static function create_transaction($order, $received_by = false, $status = null)
    {
        ...
        // Hybrid monetization fields are on Store model directly
        ...
        if ($type == 'parcel') {
            ...
            if ($order->store_discount_amount > 0  && $order->discount_on_product_by == 'vendor') {
                $should_apply_commission = ($store->store_business_model != 'subscription' || !isset($store_sub)) || ($store->commission_active);
            ...
            //final comission (hybrid model support)
            $should_apply_commission = ($store->store_business_model != 'subscription' || !isset($store_sub)) || ($store->commission_active);
            if (!$should_apply_commission) {
                $comission_on_store_amount = 0;
                $subscription_mode = 1;
                ...
                $effective_commission = $store->commission_active ? $store->comission : $comission;
                $comission_on_store_amount = ($effective_commission ? ($order_amount / 100) * $effective_commission : 0);
                $subscription_mode = ($store->store_business_model == 'subscription' && isset($store_sub)) ? 1 : 0;
        ...
        $store_amount = $store_amount + $order_amount + $order->total_tax_amount + $order->extra_packaging_amount - $comission_on_store_amount - $store_coupon_discount_subsidy - $flash_store_discount_amount - $extra_discount_amount + $fixed_delivery_fee;
        try {
