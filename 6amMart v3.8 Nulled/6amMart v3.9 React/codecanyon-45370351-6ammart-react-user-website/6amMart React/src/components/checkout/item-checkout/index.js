        ...
        const handleSuccess = (response) => {
          if (response?.data) {
            ...
            if (
              ...
              // Stripe: pagamento embedado no checkout
              if (paymentMethod === "stripe") {
                setIsStripeProcessing(true);
                stripeRef.current
                  ?.confirmPayment(response?.data?.order_id)
                  .then((result) => {
                    ...
                    if (result?.success) {
                      ...
                      dispatch(setClearCart());
                      if (token) {
                        Router.push("/profile?page=my-orders");
                      } else {
                        Router.push("/home");
                      }
                    } else {
                      toast.error(result?.error || "Pagamento falhou", {
                        position: "bottom-right",
                      });
                    }
                  });
              } else {
                // Outros gateways: redirecionar para página de pagamento
        ...
        >
          <Grid item xs={12} md={7}>
            ...
            >
              ...
              {paymentMethod === "stripe" && configData?.stripe_publishable_key && (
                <StripePaymentForm
                  ref={stripeRef}
                  publishableKey={configData?.stripe_publishable_key}
                  totalAmount={totalAmount}
                  onError={(msg) => toast.error(msg, { position: "bottom-right" })}
                />
              )}
