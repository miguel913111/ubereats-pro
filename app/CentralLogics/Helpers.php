...
//        }
    ...
    public static function setZoneIds($request): void
    {
        $zoneId = $request->header('zoneId');
        if ($zoneId) {
            $zoneId = is_string($zoneId) ? json_decode($zoneId, true) : $zoneId;
            self::$zoneIds = is_array($zoneId) ? $zoneId : [$zoneId];
        } else {
            self::$zoneIds = [];
        }
    }

    public static function getZoneIds(): array
    {
        return self::$zoneIds;
    }
    ...
    public static function getDecimalPlaces(): string
    {
        $decimal = BusinessSetting::where('key', 'digit_after_decimal_point')->first();
        $places = $decimal ? (int) $decimal->value : 0;
    ...
    public static function react_domain_status_check()
    {
        ...
        if ($data && isset($data['react_domain']) && isset($data['react_license_code'])) {
            if (isset($data['react_platform']) && $data['react_platform'] == 'nexofood') {
...
//                return rtrim($awsUrl, '/') . '/' . ltrim($awsBucket . '/' . $path . '/' . $data, '/');
    ...
    public static function seoPageList(): array
    {
        return [];
    }
