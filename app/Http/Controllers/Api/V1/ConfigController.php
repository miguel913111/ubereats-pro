...
class ConfigController extends Controller
{
    ...
    public function place_api_autocomplete(Request $request)
    {
        ...
        // Use Photon API (OpenStreetMap-based) for autocomplete
        $query = urlencode($request['search_text']);
        $url = "https://photon.komoot.io/api/?q={$query}&limit=5&lang=" . app()->getLocale();

        $ch = curl_init($url);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($ch, CURLOPT_USERAGENT, 'NexoFood/1.0 (contact@nexofood.pt)');
        ...
        if (isset($data['features'])) {
            foreach ($data['features'] as $feature) {
                ...
                $suggestions[] = [
                    'placePrediction' => [
                        'place' => $properties['osm_id'] ?? uniqid(),
                        'placeId' => $properties['osm_id'] ?? uniqid(),
                        'text' => [
                            'text' => $properties['name'] ?? $request['search_text'],
                        ],
                        'structuredFormat' => [
                            'mainText' => [
                                'text' => $properties['name'] ?? $request['search_text'],
                            ...
                            'secondaryText' => [
                                'text' => ($properties['street'] ?? '') . ' ' . ($properties['city'] ?? '') . ' ' . ($properties['country'] ?? ''),
                            ],
                        ],
                    ],
                    'lat' => $lat,
    ...
    public function distance_api(Request $request)
    {
        ...
        // Use OSRM (OpenStreetMap Routing Machine) for distance calculation
        $profile = ($request['mode'] ?? 'WALK') === 'WALK' ? 'foot' : 'driving';
        $url = "http://router.project-osrm.org/route/v1/{$profile}/{$request['origin_lng']},{$request['origin_lat']};{$request['destination_lng']},{$request['destination_lat']}?overview=false";
        ...
        if (isset($data['routes'][0])) {
            $route = $data['routes'][0];
            $distanceMeters = $route['distance'] ?? 0; // in meters
            $durationSeconds = $route['duration'] ?? 0; // in seconds

            return [
                'duration' => round($durationSeconds / 60) . ' mins',
        ...
        $lat1 = deg2rad($request['origin_lat']);
        $lat2 = deg2rad($request['destination_lat']);
        $lon1 = deg2rad($request['origin_lng']);
        $lon2 = deg2rad($request['destination_lng']);
        $dlat = $lat2 - $lat1;
        $dlon = $lon2 - $lon1;
        $a = sin($dlat / 2) * sin($dlat / 2) + cos($lat1) * cos($lat2) * sin($dlon / 2) * sin($dlon / 2);
        $c = 2 * atan2(sqrt($a), sqrt(1 - $a));
        $distanceMeters = 6371000 * $c; // Earth's radius in meters

        return [
            'duration' => round(($distanceMeters / 1000) * 10) . ' mins',
            'distanceMeters' => $distanceMeters,
            'localizedValues' => [
                'distance' => [
                    'text' => round($distanceMeters / 1000, 1) . ' km',
                ...
                'duration' => [
                    'text' => round(($distanceMeters / 1000) * 10) . ' mins',
                ],
            ],
        ];
    }
    ...
    public function place_api_details(Request $request)
    {
        ...
        // Use Nominatim for place details (lookup by osm_id)
        $url = 'https://nominatim.openstreetmap.org/lookup?osm_ids=' . urlencode($request['placeid']) . '&format=json&addressdetails=1';
        ...
        if (isset($data[0])) {
            $place = $data[0];
            return [
                'id' => $place['osm_id'] ?? $request['placeid'],
                'displayName' => [
                    'text' => $place['display_name'] ?? 'Unknown',
                ],
                'formattedAddress' => $place['display_name'] ?? 'Unknown',
                'location' => [
    ...
    public function geocode_api(Request $request)
    {
        ...
        $url = "https://nominatim.openstreetmap.org/reverse?lat={$request->lat}&lon={$request->lng}&format=json&addressdetails=1";
        ...
        // Transform Nominatim response to Google-like format for compatibility
        $addressComponents = [];
        if (isset($data['address'])) {
            foreach ($data['address'] as $type => $value) {
                $addressComponents[] = [
                    'long_name' => $value,
                    'short_name' => $value,
                    'types' => [$type],
        ...
        return [
            'results' => [
                [
                    ...
                    'geometry' => [
                        'location' => [
                            'lat' => (float) ($data['lat'] ?? $request->lat),
                            'lng' => (float) ($data['lon'] ?? $request->lng),
                        ],
                    ],
    ...
    public function direction_api(Request $request)
    {
        ...
        // Use OSRM (OpenStreetMap Routing Machine) for directions
        $profile = ($request['mode'] ?? 'DRIVE') === 'WALK' ? 'foot' : 'driving';
        $url = "http://router.project-osrm.org/route/v1/{$profile}/{$request['origin_lng']},{$request['origin_lat']};{$request['destination_lng']},{$request['destination_lat']}?overview=full&geometries=polyline";
