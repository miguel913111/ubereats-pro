...
class AddHybridModelsToStores extends Migration
{
    public function up()
    {
        Schema::table('stores', function (Blueprint $table) {
            ...
            if (Schema::hasTable('stores') && !Schema::hasColumn('stores', 'commission_active')) {
                $table->boolean('commission_active')->default(1)->after('comission');
            }
            if (Schema::hasTable('stores') && !Schema::hasColumn('stores', 'subscription_active')) {
                $table->boolean('subscription_active')->default(0)->after('commission_active');
            }
            if (Schema::hasTable('stores') && !Schema::hasColumn('stores', 'fixed_delivery_fee_active')) {
            ...
            if (Schema::hasTable('stores') && !Schema::hasColumn('stores', 'fixed_delivery_fee')) {
                $table->decimal('fixed_delivery_fee', 24, 3)->default(0)->after('fixed_delivery_fee_active');
            }
            
            // Taxas para entregador (delivery man)
            if (Schema::hasTable('stores') && !Schema::hasColumn('stores', 'driver_per_km_charge')) {
                $table->decimal('driver_per_km_charge', 24, 3)->default(0)->after('per_km_shipping_charge');
            }
            if (Schema::hasTable('stores') && !Schema::hasColumn('stores', 'driver_fixed_charge')) {
