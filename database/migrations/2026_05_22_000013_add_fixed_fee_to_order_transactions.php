...
class AddFixedFeeToOrderTransactions extends Migration
{
    public function up()
    {
        Schema::table('order_transactions', function (Blueprint $table) {
            if (Schema::hasTable('order_transactions') && !Schema::hasColumn('order_transactions', 'fixed_delivery_fee')) {
                $table->decimal('fixed_delivery_fee', 24, 3)->default(0)->after('delivery_fee_comission');
            }
            if (Schema::hasTable('order_transactions') && !Schema::hasColumn('order_transactions', 'driver_km_charge')) {
                $table->decimal('driver_km_charge', 24, 3)->default(0)->after('fixed_delivery_fee');
            }
            if (Schema::hasTable('order_transactions') && !Schema::hasColumn('order_transactions', 'driver_fixed_charge')) {
                $table->decimal('driver_fixed_charge', 24, 3)->default(0)->after('driver_km_charge');
            }
            if (Schema::hasTable('order_transactions') && !Schema::hasColumn('order_transactions', 'platform_earnings')) {
