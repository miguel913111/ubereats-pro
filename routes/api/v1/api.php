...
Route::group(['namespace' => 'Api\V1', 'middleware'=>'localization'], function () {
    ...
    Route::group(['prefix' => 'delivery-man','middleware' => 'actch:deliveryman_app' ], function () {
        ...
        Route::group(['middleware'=>['dm.api']], function () {
            ...
            // Delivery Batch / Route Optimization
            Route::get('my-batch', 'DeliveryBatchController@getMyBatch');
            Route::post('batch/start', 'DeliveryBatchController@startBatch');
            Route::post('batch/deliver-order', 'DeliveryBatchController@deliverOrder');
            Route::get('time-windows', 'DeliveryBatchController@getTimeWindows');

            // Delivery Optimization
            Route::post('delivery-optimization/suggest-batch', 'DeliveryOptimizationController@suggestBatch');
            Route::post('delivery-optimization/estimate-window', 'DeliveryOptimizationController@estimateWindow');
    ...
    Route::group(['prefix' => 'vendor', 'namespace' => 'Vendor', 'middleware'=>['vendor.api','actch:vendor_app']], function () {
       ...
       // advertisement
        ...
        // Dine-in
        Route::group(['prefix'=>'dine-in'], function(){
            Route::get('tables', 'VendorDineInController@getTables');
            Route::post('tables/store', 'VendorDineInController@storeTable');
            Route::put('tables/update/{id}', 'VendorDineInController@updateTable');
            Route::delete('tables/delete/{id}', 'VendorDineInController@deleteTable');
            Route::get('reservations', 'VendorDineInController@getReservations');
            Route::post('reservations/confirm/{id}', 'VendorDineInController@confirmReservation');
            Route::post('reservations/complete/{id}', 'VendorDineInController@completeReservation');
            Route::post('reservations/cancel/{id}', 'VendorDineInController@cancelReservation');
        });

        // Store Delivery Zones
        Route::group(['prefix'=>'store-delivery-zone'], function(){
            Route::get('list', 'VendorDineInController@getDeliveryZones');
            Route::post('store', 'VendorDineInController@storeDeliveryZone');
            Route::delete('delete/{id}', 'VendorDineInController@deleteDeliveryZone');
        });

        // Hybrid Pricing
        Route::group(['prefix'=>'hybrid-pricing'], function(){
            Route::get('my-models', 'VendorHybridPricingController@getMyPricingModels');
            Route::post('update-models', 'VendorHybridPricingController@updatePricingModels');
            Route::post('update-driver-rates', 'VendorHybridPricingController@updateDriverRates');
            Route::post('simulate', 'VendorHybridPricingController@simulateEarnings');
        });

        // Self Delivery
        Route::group(['prefix'=>'self-delivery'], function(){
            Route::get('delivery-men', 'VendorSelfDeliveryController@getDeliveryMen');
            Route::post('delivery-men/store', 'VendorSelfDeliveryController@storeDeliveryMan');
            Route::put('delivery-men/update/{id}', 'VendorSelfDeliveryController@updateDeliveryMan');
            Route::delete('delivery-men/delete/{id}', 'VendorSelfDeliveryController@deleteDeliveryMan');
            Route::post('assign-order', 'VendorSelfDeliveryController@assignOrder');
        });

        // Store Delivery Man Auth & Orders
        Route::group(['prefix'=>'store-delivery-man'], function(){
            Route::post('login', 'VendorSelfDeliveryController@login')->withoutMiddleware(['vendor.api','actch:vendor_app']);
            Route::get('orders', 'VendorSelfDeliveryController@getAssignedOrders')->withoutMiddleware(['vendor.api']);
            Route::post('update-order-status', 'VendorSelfDeliveryController@updateOrderStatus')->withoutMiddleware(['vendor.api']);
        });

        // Dine-in QR
        Route::group(['prefix'=>'dine-in-qr'], function(){
            Route::post('check-in', 'DineInQrController@checkIn');
            Route::post('get-reservation', 'DineInQrController@getReservationByQr');
        });
    ...
    Route::group(['middleware'=>['module-check']], function(){
        Route::group(['prefix' => 'customer', 'middleware' => 'auth:api'], function () {
            ...
            Route::group(['prefix'=>'gift-card'], function() {
                Route::get('list', 'GiftCardController@list');
                Route::post('apply', 'GiftCardController@apply');
                Route::post('purchase', 'GiftCardController@purchase');
                Route::post('redeem', 'GiftCardController@redeem');
                Route::get('my-gift-cards', 'GiftCardController@myGiftCards');
            });

            Route::group(['prefix'=>'document-verification'], function() {
                Route::post('store', 'DocumentVerificationController@store');
                Route::get('my-documents', 'DocumentVerificationController@myDocuments');
                Route::get('check-status', 'DocumentVerificationController@checkStatus');
            });

            Route::group(['prefix'=>'dine-in'], function() {
                Route::get('stores', 'DineInController@getStores');
                Route::get('tables', 'DineInController@getTables');
                Route::get('check-availability', 'DineInController@checkAvailability');
                Route::post('book', 'DineInController@bookTable');
                Route::get('my-reservations', 'DineInController@myReservations');
                Route::post('cancel/{id}', 'DineInController@cancelReservation');
            });

            Route::group(['prefix'=>'store-delivery-zone'], function() {
                Route::get('list', 'StoreDeliveryZoneController@getByStore');
                Route::get('check', 'StoreDeliveryZoneController@checkInZone');
            });

            Route::group(['prefix'=>'dine-in-qr'], function() {
                Route::get('get/{reservationId}', 'DineInQrController@getQrCode');
            });

            Route::group(['prefix'=>'stories'], function() {
                Route::get('list', 'StoryController@index');
                Route::get('view/{id}', 'StoryController@view');
            });

            Route::group(['prefix'=>'cashback'], function() {
                Route::get('calculate', 'CashBackController@calculate');
                Route::get('list', 'CashBackController@list');
                Route::get('my-history', 'CashBackController@myCashBackHistory');
            });
